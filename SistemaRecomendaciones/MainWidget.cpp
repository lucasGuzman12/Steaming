...
// Implementa la interfaz principal y traduce acciones de la UI en llamadas
// concretas a Recomendador.
...
namespace
{
// Helper local para mostrar resultados largos en un dialogo reutilizable.
...
{
    // Configura toda la estructura visual y conecta los botones con el
    // recomendador y los helpers de presentacion.
...
{
    // Pide el Steam ID al usuario y muestra la biblioteca en formato JSON.
    bool ok = false;
...
{
    // Ejecuta el flujo principal: Steam -> OpenAI -> APIs de detalle.
    bool ok = false;
    const QString steamId = QInputDialog::getText(
        this,
        "Recomendador",
...
{
    // Permite probar la API de YouTube de manera independiente.
    bool ok = false;
...
{
    // Permite probar la API de peliculas de forma aislada.
    bool ok = false;
...
{
    // Permite probar la API de libros de forma aislada.
    bool ok = false;
...
{
    // Navega por el siguiente bloque cacheado de todas las categorias.
...
{
    // Navega solo por el bloque siguiente de peliculas.
    if (currentSteamId.isEmpty()) {
        statusLabel->setText("Por favor genera recomendaciones primero.");
        return;
    }
...
{
    // Navega solo por el bloque siguiente de libros.
    if (currentSteamId.isEmpty()) {
        statusLabel->setText("Por favor genera recomendaciones primero.");
        return;
    }
...
{
    // Navega solo por el bloque siguiente de juegos.
    if (currentSteamId.isEmpty()) {
        statusLabel->setText("Por favor genera recomendaciones primero.");
        return;
    }
...
{
    // Navega solo por el bloque siguiente de videos.
    if (currentSteamId.isEmpty()) {
        statusLabel->setText("Por favor genera recomendaciones primero.");
        return;
    }
...
{
    // Muestra un ejemplo de salida basado en PaqueteRecomendaciones.
...
{
    // Ejecuta una solicitud de texto libre para mostrar el uso de
    // SolicitudChatGpt fuera del modo de recomendaciones.
    bool ok = false;
...
{
    // Refresca el texto de estado cuando el recomendador avisa que ya termino.
...
{
    // Presenta una salida textual simple ocultando la lista de resultados.
...
{
    // Formatea un objeto JSON con sangria y lo muestra en el panel de texto.
    statusLabel->setText(titulo);
    resultList->clear();
    resultList->setVisible(false);
    outputEdit->setVisible(true);
...
{
    // Renderiza la salida principal del recomendador separada por categorias.
...
{
    // Presenta una sola categoria con resumen textual y lista visual.
    statusLabel->setText(titulo);
    outputEdit->setVisible(true);
...
{
    // Inserta un encabezado y luego cada item con texto e imagen opcional.
...
{
    // Descarga miniaturas de forma asincronica y las asigna al item cuando
    // estan listas, sin bloquear la interfaz.
...
{
    // Delega en Recomendador el resumen textual para mantener una sola fuente
    // de verdad sobre como describir los resultados.
