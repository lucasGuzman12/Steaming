...
// Implementa una capa de servicios desacoplada para consultar APIs puntuales
// y una variante dedicada del flujo de recomendaciones con OpenAI.
#include <QJsonArray>
...
namespace
{
// Helper comun para devolver errores con una estructura JSON uniforme.
...
// Define el schema de una recomendacion individual generada por OpenAI.
...
// Envuelve un itemSchema en una coleccion de tipo array.
...
{
    // Recupera la biblioteca del usuario y enriquece los juegos mas relevantes.
...
{
    // Consulta appdetails para sumar portada, descripcion y generos.
    if (appId <= 0) {
        return QJsonObject();
    }
...
{
    // Devuelve una representacion textual simple util para pruebas o demos.
...
{
    // Busca juegos en el store y uniforma los datos de salida.
...
{
    // Consulta Google Books y devuelve un formato compatible con la UI.
    const QString tituloNormalizado = titulo.trimmed();
    if (tituloNormalizado.isEmpty()) {
        return crearErrorApi(
            "libros",
...
{
    // Consulta TMDB y agrega el director a cada pelicula devuelta.
    const QString nombreNormalizado = nombre.trimmed();
    if (nombreNormalizado.isEmpty()) {
        return crearErrorApi(
            "peliculas",
...
{
    // Consulta la API de YouTube y transforma la respuesta a un formato uniforme.
    const QString textoNormalizado = texto.trimmed();
    if (textoNormalizado.isEmpty()) {
        return crearErrorApi(
            "youtube",
...
SolicitudRecomendacionesChatGpt::SolicitudRecomendacionesChatGpt(
    ...
    : contextoSteam(contextoSteam),
      ...
      modelo(modelo)
{
    // Solo guarda el contexto y la configuracion necesaria para construir el payload.
}
...
{
    // Ejecuta una solicitud estructurada a OpenAI y devuelve el JSON interpretado.
