...
// Implementa el flujo completo de recomendaciones y el soporte de cache local.
#include <QDateTime>
#include <QDir>
...
namespace
{
// Intenta parsear un string a objeto JSON sin propagar errores.
...
// Interpreta banderas string de configuracion de forma tolerante.
...
// Reconoce estados intermedios de la API Responses de OpenAI.
...
{
    // Recupera la biblioteca visible del usuario y opcionalmente enriquece
    // los juegos mas relevantes con detalle adicional del store.
...
{
    // Flujo central: biblioteca -> contexto -> OpenAI -> hidratacion
    // -> cache -> seleccion del bloque visible para la UI.
...
{
    // Traduce la salida JSON del recomendador a modelos C++ agrupados.
...
{
    // Reutiliza el cache local para mostrar nuevas tandas sin repetir requests.
...
{
    // Resume la biblioteca en una estructura mas compacta para darsela a OpenAI.
...
{
    // Toma sugerencias abstractas de OpenAI y busca detalle real en Google Books.
    QJsonArray resultados;
...
{
    // Toma sugerencias abstractas de OpenAI y busca detalle real en TMDB.
    QJsonArray resultados;
    for (const QJsonValue &valor : sugerencias) {
        const QJsonObject sugerencia = valor.toObject();
        const QString query = sugerencia.value("query").toString(
...
{
    // Toma sugerencias abstractas de OpenAI y busca detalle real en Steam Store.
    QJsonArray resultados;
    for (const QJsonValue &valor : sugerencias) {
        const QJsonObject sugerencia = valor.toObject();
        const QString query = sugerencia.value("query").toString(
...
{
    // Toma sugerencias abstractas de OpenAI y busca detalle real en YouTube.
    QJsonArray resultados;
    for (const QJsonValue &valor : sugerencias) {
        const QJsonObject sugerencia = valor.toObject();
        const QString query = sugerencia.value("query").toString(
...
{
    // Primero intenta resolver desde cache; si no alcanza, consulta OpenAI y
    // guarda un bloque mas grande para permitir paginacion posterior.
    QJsonObject cache = cargarCache(steamId);
...
{
    // Construye la solicitud estructurada para OpenAI y reintenta si la
    // respuesta llega incompleta o no cumple la cantidad minima.
...
{
    // Ejecuta la solicitud en primer plano usando la API Responses.
    const QString apiKey = DataManager::cargarConfiguracion("OPENAI_API_KEY");
    if (apiKey.trimmed().isEmpty()) {
        return crearErrorObjeto(
            "chatgpt",
...
{
    // Variante asincronica para respuestas largas que pueden tardar en completarse.
    const QString respuesta = apiChatGpt.post("responses", solicitud.crearPayload());
    const QJsonObject respuestaObjeto = parsearObjetoJson(respuesta);
...
{
    // Hace polling hasta que OpenAI termine o se alcance el timeout maximo.
...
{
    // Consulta appdetails para enriquecer juegos con descripcion, portada y precio.
    if (appId <= 0) {
        return QJsonObject();
    }
...
{
    // Traduce una busqueda textual a un formato uniforme de detalle de libro.
...
{
    // Busca peliculas en TMDB y completa director, sinopsis y poster.
...
{
    // Busca juegos en la tienda de Steam y agrega informacion de detalle.
...
{
    // Busca videos en YouTube y unifica la respuesta para la UI.
    const QString textoNormalizado = texto.trimmed();
    if (textoNormalizado.isEmpty()) {
        return crearErrorObjeto(
            "youtube",
...
{
    // Implementa paginacion circular sobre una lista completa de sugerencias.
...
{
    // Lee el offset actual de una categoria, devuelve el siguiente bloque y
    // persiste el nuevo offset en cache.
    QJsonObject recomendaciones = cache.value("recomendaciones").toObject();
...
{
    // Lleva una cuenta simple de usos del recomendador en AppData.
...
{
    // Actualiza el contador y deja registrada la ultima fecha de uso.
...
{
    // Carga de disco el cache del usuario indicado, si existe.
...
{
    // Persiste en AppData las recomendaciones y offsets del usuario.
    const QString dirApp =
        QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
    QDir().mkpath(dirApp);
...
{
    // Genera el nombre de archivo del cache asociado a un Steam ID.
    const QString dirApp =
        QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
